 ...
 * 
 */
public class JavaTypeValidatorProvider {
	 ...
	 */
	private final List<JavaTypeValidatorProvider> parameters = new ArrayList<JavaTypeValidatorProvider>();
	 ...
	 */
	public JavaTypeValidatorProvider(String typeAsString)
			throws IllegalStateException {
		...
		if (ltIndices.length == 0) {
			...
			for (int i = 0; i < firstLevelCommaIndices.size() - 1; i++) {
				this.parameters.add(new JavaTypeValidatorProvider(
	...
	public List<JavaTypeValidatorProvider> getTypeParameters() {
		return parameters;
	}
