 ...
 * @author
 */
public class ProgramBasePropertySection extends AbstractPropertySection implements IFilter {
		...
		if (editingDomain != null && newEObject != null && newEObject != eObject) {
			...
			if (eObject != null) {
				...
				if (this.propertiesEditionComponent != null) {
					...
					this.editionPart = propertiesEditionComponent.getPropertiesEditionPart(1, ProgramPropertiesEditionComponent.BASE_PART); //$NON-NLS-1$
					if (editionPart instanceof IFormPropertiesEditionPart) {
						...
						parent.layout();
						propertiesEditionComponent.initPart(propertiesEditionComponent.translatePart(ProgramPropertiesEditionComponent.BASE_PART), 1, eObject);
					}
				}
			}
		}
	...
	public boolean select(Object toTest) {
		...
		return eObjectToTest != null && eObjectToTest.eClass() == ParallelJPackage.eINSTANCE.getProgram();
	}
