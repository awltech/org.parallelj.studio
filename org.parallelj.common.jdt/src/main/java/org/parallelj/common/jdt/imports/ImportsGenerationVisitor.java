 ...
 */
package org.parallelj.common.jdt.imports;
...
public class ImportsGenerationVisitor extends ASTVisitor {
		...
		try {
			...
			Activator.getDefault().getLog().log(
					new Status(Status.ERROR,
							Activator.PLUGIN_ID,
					"Exception encountered while generating imports", t));
		}
		return false;
	}
	...
	private void addMissingImports(CompilationUnit unit) {
		unit.imports().addAll(resolvedImports.values());
	}
	...
	public boolean visit(QualifiedName node) {
		...
		if (binding != null) {
			...
			if (binding.getKind() == IBinding.TYPE
					&& !((ITypeBinding) binding).isRecovered()) {
				checkQualifiedType(node.getParent(), node,
	...
	private void checkType(ASTNode node, Name typeName, ITypeBinding typeBinding) {
		if (typeName.isSimpleName())
			...
			this
					.checkQualifiedType(node, (QualifiedName) typeName,
							typeBinding);
	}
	 ...
	 *            : Type Binding
	 */
	private void checkSimpleType(ASTNode node, SimpleName simpleName,
			ITypeBinding typeBinding) {

		// If type binding is null, recovered or type corresponds to primitive:
		// returns.
		if (typeBinding == null || typeBinding.isRecovered()
				|| typeBinding.getPackage() == null)
			return;
	 ...
	 */
	private boolean shouldAddImport(ASTNode node, String typeName,
			String packageName, String fullyQualifiedName) {
		...
		return packageName != null
				&& !packageName
						.equals(this.typesToPackageBinding.get(typeName))
				&& !this.resolvedImports.containsKey(fullyQualifiedName)
				&& !packageName.equals("java.lang")
				&& !packageName.equals(currentPackageName);
	}
