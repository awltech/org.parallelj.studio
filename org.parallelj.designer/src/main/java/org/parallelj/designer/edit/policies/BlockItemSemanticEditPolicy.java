...
import org.parallelj.designer.edit.commands.LinkCreateCommand;
import org.parallelj.designer.edit.commands.LinkReorientCommand;
 ...
 * @generated
 */
public class BlockItemSemanticEditPolicy extends
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (ParallelJVisualIDRegistry.getVisualID(incomingLink) == LinkEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (ParallelJVisualIDRegistry.getVisualID(outgoingLink) == LinkEditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(
	...
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (ParallelJVisualIDRegistry.getVisualID(node)) {
			case BlockBlockCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit
						.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (ParallelJVisualIDRegistry.getVisualID(cnode)) {
					case BlockProcedureEditPart.VISUAL_ID:
						for (Iterator<?> it = cnode.getTargetEdges().iterator(); it
								.hasNext();) {
							Edge incomingLink = (Edge) it.next();
							if (ParallelJVisualIDRegistry
	...
	protected Command getStartCreateRelationshipCommand(
		...
		if (ParallelJElementTypes.Link_4001 == req.getElementType()) {
