...
public class BlockProcedureExtendedEditPart extends
		BlockProcedureEditPart {
	...
	public BlockProcedureExtendedEditPart(View view) {
		super(view);
		updateColor(preferredColor, false);
	}

	/**
	 * Fixing the height, width and location of the BlockProcedure node as
	 * per parent Block node and updating diagram with last saved color when
	...
	protected void refreshBounds() {
		...
		if (this.getParent() != null) {
			...
			int blockWidth = this.getFeatureValue(
					(BlockExtendedEditPart) root,
			...
			if (blockWidth > 0) {
				...
				// compartment
				BoundsRefreshment.refreshBounds(this, blockWidth - 18,
						procedureHeight);
			...
			// set location as per sequence number in block
	 ...
	 * at last or empty space from Block compartment to add procedures.
	 */
	...
	/**
	 * Creates rectangle for BlockProcedure node, with color gradient
