...
public class PipelineExtendedEditPart extends PipelineEditPart {
	...
	/**
	 * Fixing the height and width of the Pipeline node and updating diagram
	...
	protected void refreshBounds() {
		...
		if (innerHeight != 35) {
			...
			Pipeline pipeline = (Pipeline) (((View) this.getModel())
					.getElement());
			if (pipeline.getProcedures().size() <= 1) {
				BoundsRefreshment.refreshBounds(this, null, 89);
			} else {
				BoundsRefreshment.refreshBounds(this, null, 89 + ((pipeline
	...
	/**
	 * This will remove the bottom margin, called at the time when pipeline is
	 * in fold mode.
	 */
			...
			if (rectangleFigure.isVisible()) {
				BoundsRefreshment
						.refreshBounds(this, this.getSize().width - 2,
								89 + ((this.getPipeline().getProcedures()
										.size() - 1) * 45));
			} else {
				BoundsRefreshment.refreshBounds(this, this.getSize().width - 2,
						35);
